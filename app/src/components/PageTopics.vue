...
import { useFromRoute, useSlug, useWorkspace } from '@/composables'
...
const fetchTopicTweets = async () => {
  ...
  try {
    loading.value = true
    tweets.value = await fetchTweets(workspace, [topicFilter(slugTopic.value)])
...
<template>
  ...
  <div v-if="viewedTopic">
    <TweetForm @added="addTweet" :forced-topic="viewedTopic"></TweetForm>
